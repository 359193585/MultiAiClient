 ...
 *
 * 项目名称： MultiAIClient  
 * 文 件 名:  InjectAndSubmitToGemini.cs
...
{
    internal partial class InjectGemini
    {

        internal static async Task<bool> InjectModule(WebView2 targetWebView)
        {
            string script = await Scripts.LoadInjectionScriptAsync("GeminiSend.js");
            return await Scripts.RunInjectScript(targetWebView, script, "Gemini 脚本");
        }
        ...
        internal static async Task<bool> SubmitQuestion(WebView2 targetWebView, string text)
        {
            ...
            string script = $"window.__my_injected__.geminiSend.tryCompleteOperation(`{cssSelector}`,`{injectText}`,`{sendBtnSelector}`)";
            return await Scripts.RunInjectScript(targetWebView, script, "Gemini 发送脚本");
        }
